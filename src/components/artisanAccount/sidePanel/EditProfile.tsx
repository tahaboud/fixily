...
import { validateArtisanEditProfile } from "../../../validators/artisanEditProfile";
import { ArtisanEditProfileValidationErrors } from "../../../validators/types";
import {
  SnackbarContext,
  SnackbarContextType,
...
const EditProfile = ({
  ...
  const [errors, setErrors] = useState<ArtisanEditProfileValidationErrors>({});
  ...
  const handleSave = () => {
    ...
    const { isValid, validationErrors } = validateArtisanEditProfile(newValues);
    if (isValid) {
      if (token) {
        const changedValues: {
          email?: string;
